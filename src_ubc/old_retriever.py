...
class DefaultRetriever(Retriever):
    ...
    def process(self, query: list[Query], k=20, dense_weight=.2, sparse_weight=.8) -> list[dict]:
    ...
    def _retrieve_with_text(self, query: str, dense_weight=.5, sparse_weight=.5,  k=20) -> list[dict]:
        ...
        sparse_results = self.sparse_db.query(
            ...
            include_values=True,
            include_metadata=True,
            sparse_vector=self.sparse_embedding_model.encode_text([query])[
                0
            ],
        )

        # Merge dense and sparse results
        merged_results = self._merge_results(dense_results, sparse_results, dense_weight, sparse_weight)
